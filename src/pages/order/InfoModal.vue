    ...
    <el-drawer title="订单详情" v-model="dialogVisible" direction="rtl" size="50%">
        ...
        <el-card shadow="never" class="mb-3">
            ...
            <div class="flex mb-2" v-for="(item, index) in info.order_items" :key="index">
                <el-image :src="item.goos_item?.cover ?? ''" fit="fill" :lazy="true" style="width: 60px;height: 60px;">

                </el-image>
                <div class="ml-2 text-sm">
                    <p>{{ item.goos_item?.title ?? "商品已被删除" }}</p>
                    ...
                    <p>
                        ...
                        <span class="text-xs text-gray-500">x{{ item.name }}</span>
                    </p>
                </div>
            </div>
            <el-form label-width="80px">
