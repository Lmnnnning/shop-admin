<template>
    <div v-loading="loading" class="bg-white p-4 rounded">
                        ...
                        <el-table-column label="支付方式" align="left">
                            <template #default="{ row }">
                                <div class="flex items-center">
                                    <el-image :src="row.src" fit="fill" :lazy="true" style="width: 40px;
                                    height: 40px;" class="rounded mr-2"></el-image>
                                    <div>
                                        <h6>{{ row.name }}</h6>
        ...
        <FormDrawer ref="fromDrawerRef" title="配置" @submit="submit">
            ...
            <el-form v-else :model="form" label-width="80px">
                ...
                <el-form-item label="app_id">
                    <el-input v-model="form.wxpay.app_id" placeholder="appid" style="width: 90%;"></el-input>
                </el-form-item>
...
function getData() {
    ...
    getSysconfig().then(res => {
        ...
        if (form.password_encrypt) {
            form.password_encrypt = form.password_encrypt.split(",")
        } else {
            form.password_encrypt = [] // 设为默认空数组
        }
    }).finally(() => {
        loading.value = false
    })
