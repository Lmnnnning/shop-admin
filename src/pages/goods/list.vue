<template>
    <div>
        ...
        <el-card shadow="never" class="border-0">
            <!-- 搜索 -->
            <Search :model="searchForm" @search="getData" @reseet="resetSearchForm">
                ...
                <template #show>
                    <SearchItem label="商品分类">
                        <el-select v-model="searchForm.category_id" placeholder="请选择商品分类" clearable>
                            <el-option v-for="item in category_list" :key="item.value" :label="item.name"
                                :value="item.id">
                            </el-option>
                        </el-select>
                    </SearchItem>
                        ...
                        <div class="flex">
                            ...
                            <div class="flex-1">
                                ...
                                <div>
                                    ...
                                    <span class="text-gray-500 tx-xs">￥{{ row.min_oprice }}</span>
                                </div>
                                <p class="text-gray-4 00 text-xs mb-1">分类:{{ row.category ? row.category.name : "未分类" }}
                                </p>
                                <p class="text-gray-4 00 text-xs">创建时间:{{ row.create_time }}</p>
                            </div>
                ...
                <el-table-column label="操作" align="center">
                    <template #default="scope">
                        <div v-if="searchForm.tab != 'delete'">
                            <el-button class="px-1" type="primary" size="small" @click="handleEdit(scope.row)"
                                text>修改</el-button>
            ...
            <FormDrawer ref="formDrawerRef" :title="drawerTitle" @submit="handleSubmit">
                <el-form :model="form" ref="formRef" :rules="rules" label-width="80px" :inline="false">
                    ...
                    <el-form-item label="最低销价" prop="min_price">
...
getCategoryList()
    .then(res => category_list.value = res)
...
const handleRestoreGoods = () => useMutiAction(restoreGoods, "恢复")

const handleDestoryGoods = () => useMutiAction(destroyGoods, "彻底删除")

function useMutiAction(func, msg) {
    //二者只有提示和方法不一样进行封装
    loading.value = true
